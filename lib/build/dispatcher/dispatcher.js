...
class Dispatcher {
   ...
   * @param {string[]|undefined} dirsToinject - reference to dirs that contains files to be injected in worker memory
   */
  constructor(preset, mode, entry, useNodePolyfills, dirsToinject) {
    this.preset = preset;
    this.mode = mode;
    this.entry = entry;
    this.useNodePolyfills = useNodePolyfills;
    this.dirsToinject = dirsToinject;
  ...
  run = async () => {
    ...
    try {
      ...
      if (this.dirsToinject && this.dirsToinject.length > 0) {
        config.contentToInject = await injectFilesInMem(this.dirsToinject);
      }
