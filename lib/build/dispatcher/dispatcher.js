...
class Dispatcher {
  ...
  run = async () => {
    ...
    // TODO: read build config file and set this values
    const buildConfigFile = {
      fs: {
        dirsToInject: ['.faststore/@generated/graphql/'],
        pathPrefixToRemove: '.faststore/',
      },
    };
    ...
    try {
      ...
      const { dirsToInject, pathPrefixToRemove } = buildConfigFile.fs;
      if (dirsToInject && dirsToInject.length > 0) {
        config.contentToInject = await injectFilesInMem(dirsToInject);

        const prefix =
          pathPrefixToRemove &&
          typeof pathPrefixToRemove === 'string' &&
          pathPrefixToRemove !== ''
            ? `'${pathPrefixToRemove}'`
            : `''`;
        config.contentToInject = `const FS_PATH_PREFIX_TO_REMOVE = ${prefix};\n${config.contentToInject}`;
      }
