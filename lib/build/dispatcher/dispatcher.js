...
import generateTimestamp from '../../utils/generateTimestamp/generateTimestamp.utils.js';
...
async function checkNodeModules() {
  console.log('checkNodeModules');
  let projectJson;
  try {
    projectJson = getProjectJsonFile('package.json');
    console.log('Find package.json');
  } catch (error) {
    console.log('Not find package.json', error.code, error);
...
class Dispatcher {
   ...
   * @param generateTimestamp
   * @param buildId
   */
  constructor(config, buildId = generateTimestamp()) {
    /* configuration */
  ...
  async builder(config) {
    ...
    switch (config.builder) {
      case 'webpack':
        selectedBuilder = new Webpack(config);
        break;
      case 'esbuild':
        selectedBuilder = new Esbuild(config);
        break;
      default:
        selectedBuilder = new Webpack(config);
        break;
    }
    await selectedBuilder.run();
  }
