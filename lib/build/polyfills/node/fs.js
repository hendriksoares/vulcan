...
const PATH_PREFIX = '.faststore/';
globalThis.__CHANGED__ = false;
function change() {
  if (!globalThis.__CHANGED__) {
    Object.keys(MEM_FILES).forEach((e) => {
      const newKey = e.replace(PATH_PREFIX, '');
      MEM_FILES[newKey] = MEM_FILES[e];
      delete MEM_FILES[e];
    });
    globalThis.__CHANGED__ = true;

    console.log('\n\n### MEM_FILES:');
    console.log(Object.keys(MEM_FILES));
  }
}
