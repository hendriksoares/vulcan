...
async function run(nextVersion, buildContext) {
  ...
  if (gte(version, '12.3.0') && lt(version, '12.4.0')) {
    ...
    const defaultNextDirsToInclude = ['./.next', './public'];
    // TODO : Inject dir and files based on vulcan cli args (--serverFiles and --storageFiles)
    const customDirsToInclude = ['./.faststore/@generated/graphql'];
    // STATICS
    const buildStatic = new BuildStatic({
      rootDir,
      includeDirs: [
        ...new Set([...customDirsToInclude, ...defaultNextDirsToInclude]),
      ],
